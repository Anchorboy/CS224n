...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    N = dcost.shape[0]
    D = predicted.shape[0]
    grad = np.dot(dcost.reshape(N, 1), predicted.reshape(1, D))
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    zo = np.dot(uo, predicted)
    neg_zK = np.dot(-uK, predicted)
    cost = - (
        np.log(sigmoid(zo)) +
        np.sum( np.log(sigmoid(neg_zK)) )
    )
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    for cwd in contextWords:
        idx = tokens[cwd]
        cost_, gradPred, grad = word2vecCostAndGradient(vc, idx, outputVectors, dataset)
        cost += cost_
        gradOut += grad
        gradIn[c, :] += gradPred
    ### END YOUR CODE
...
def cbow(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    cost, gradPred, grad = word2vecCostAndGradient(v_hat,
                                                      c,
                                                      outputVectors,
                                                      dataset)
    gradOut = grad
