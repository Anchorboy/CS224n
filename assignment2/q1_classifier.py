...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        # b = batch_size * n_classes
        W_shape = (self.config.n_features, self.config.n_classes)
        b_shape = (self.config.n_classes,)
        # remember ->> tf.zeros IS NOT VARIABLE
        W = tf.Variable(tf.zeros(shape=W_shape, dtype=tf.float32), name='W')
        b = tf.Variable(tf.zeros(shape=b_shape, dtype=tf.float32), name='b')
