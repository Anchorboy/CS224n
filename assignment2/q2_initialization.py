...
def xavier_weight_init():
    ...
    def _xavier_initializer(shape, **kwargs):
        ...
        ### YOUR CODE HERE
        if len(shape) == 1:
            epison = np.sqrt(6) / np.sqrt(shape)
            out = tf.random_uniform(shape=shape, minval=-epison, maxval=epison, dtype=tf.float32, seed=123)
            # out = tf.constant(value=np.sqrt(6)/np.sqrt(shape), dtype=tf.float32, shape=shape)
        else:
            s = 1
            for i in shape:
                s *= i
            epison = np.sqrt(6) / np.sqrt(s)
            out = tf.random_uniform(shape=shape, minval=-epison, maxval=epison, dtype=tf.float32, seed=123)
            # out = tf.constant(value=np.sqrt(6)/np.sqrt(shape[0] * shape[1]), dtype=tf.float32, shape=shape)
        ### END YOUR CODE
        return out
