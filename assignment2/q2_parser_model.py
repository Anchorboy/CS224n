...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE
        self.input_placeholder = tf.placeholder(dtype=tf.int32, shape=(None, self.config.n_features), name='input')
        self.labels_placeholder = tf.placeholder(dtype=tf.float32, shape=(None, self.config.n_classes), name='labels')
    ...
    def create_feed_dict(self, inputs_batch, labels_batch=None, dropout=1):
        ...
        """
        ### YOUR CODE HERE
        # inputs_batch_dropout = tf.nn.dropout(inputs_batch, dropout)
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE\
        # embedding size = (5157, 50) (V, d)
        # print self.pretrained_embeddings.shape

        # old with emb reshape
        # Tensor("embedding_lookup:0", shape=(?, 36, 50), dtype = float32)
        # Tensor("Reshape:0", shape=(257850,), dtype=float32)
        # new with looked_up reshape
        # Tensor("Reshape:0", shape=(?, 1800), dtype=float32)

        emb = tf.Variable(self.pretrained_embeddings)
        # provide corresponding vector in embedding table to the words in mini_batch!!!!
        looked_up = tf.nn.embedding_lookup(emb, self.input_placeholder)
        embeddings = tf.reshape(tensor=looked_up, shape=[-1, self.config.n_features * self.config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        # x = (None, 1800) = (None, n_features * embed_size)
        # W = (n_features * embed_size, hidden_size)
        # b1 = (hidden_size)
        # U = (hidden_size, n_classes)
        # b2 = (n_classes)
        ...
        W = tf.Variable(init(shape=[self.config.n_features * self.config.embed_size, self.config.hidden_size]), name='W')
        # W = tf.Variable(tf.truncated_normal(shape=[self.config.n_features * self.config.embed_size, self.config.hidden_size], stddev=0.1))
        b1 = tf.Variable(tf.zeros(shape=[self.config.hidden_size]), name='b1')
        U = tf.Variable(init(shape=[self.config.hidden_size, self.config.n_classes]), name='U')
    ...
    def add_loss_op(self, pred):
        ...
        loss = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=pred))
        ### END YOUR CODE
        return loss
