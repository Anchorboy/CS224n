...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        l_start, r_end = window_size-1, len(sentence)-window_size+1
        for idx in range(l_start, r_end):
            wlist = []
            for w in range(-window_size, 2*window_size):
                if idx + w == -1:
                    wlist += start
                elif idx + w == len(sentence):
                    wlist += end
                else:
                    wlist += sentence[idx + w]
            windowed_data += [(wlist, labels[idx])]
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable('W', (self.config.n_window_features * self.config.embed_size, self.config.hidden_size), initializer=init(seed=1))
        b1 = tf.get_variable('b1', (self.config.hidden_size), initializer=init(seed=2))
        h = tf.nn.relu(tf.matmul(x, W) + b1)
        h_drop = tf.nn.dropout(h, dropout_rate)
        U = tf.get_variable('U', (self.config.hidden_size, self.config.n_classes), initializer=init(seed=3))
        b2 = tf.get_variable('b2', (self.config.n_classes), initializer=init(seed=4))
