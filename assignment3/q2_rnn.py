...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        sentence, labels = sentence[:max_length], labels[:max_length]
        mask = []
        sent_len = len(sentence)
        sentence += [zero_vector for _ in range(max_length - sent_len)]
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("Layer1"):
            U = tf.get_variable('U', shape=(Config.hidden_size, Config.n_classes), initializer=init(seed=1))
            b2 = tf.get_variable('b2', shape=(Config.n_classes), initializer=init(seed=2))
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o_t, h_t = cell(x[:,time_step,:], h, scope="RNN")
